...
from book.views import index, create_book, shop, register_mobile, register, register_json, response, response_info, \
    redirect_url, set_cookies, get_cookies, set_sessions, get_sessions, login, LoginView, MyOrderView
...
urlpatterns = [
    ...
    # <------------LoginView视图函数------------>
    path('login_demo/', LoginView.as_view()),

    # <------------OrderView视图函数------------>
    path('order_demo/', MyOrderView.as_view())
]